...
Public Class MetodoProductoDA
	...
	Public Function CargarTodosProductos(ByVal parametros As Dictionary(Of String, String), orderby As List(Of Tuple(Of Integer, String, Integer)), ascOrDesc As String, Optional esServicio As String = "")
		...
		End If
		ds = New DataSet
		sqlStr = "set dateformat dmy select p.Id,p.Nombre,p.Foto,cast((( P.precio * P.utilidad)/100+(P.precio)) as decimal(10,2)) as 'Precio de Venta',prov.Nombre as Proveedor, prov.id, p.FechaAlta as 'Fecha de Alta', p.CategoriaId, P.precio as 'Precio de Costo', p.utilidad from Productos as p inner join proveedores as prov on prov.id = p.ProveedorId " + innerJoin
	...
	Public Function CargarGrillaStock(parametros As Dictionary(Of String, String), orderby As List(Of Tuple(Of Integer, String, Integer)), ascOrDesc As String) As DataSet
		...
		Dim sqlStr = "set dateformat dmy SELECT p.id, p.Nombre, SUM(m.cantidad) as 'Stock Actual', p.StockMax as 'Stock Maximo',p.StockMin as 'Stock Minimo', pro.Nombre + ' ' + pro.apellido as 'Nombre Proveedor', pro.id,p.precio as 'Precio producto' FROM Productos as p left join MovimientosStock as m on m.ProductoId = p.Id inner join proveedores pro on pro.id = p.proveedorid " + text + "  GROUP BY p.Nombre, p.Id, p.StockMax, p.StockMin, pro.Nombre,pro.Apellido, pro.id,p.precio "
	...
	Public Sub ModificarUtilidad(listOfProductos As IEnumerable(Of Tuple(Of Integer, Boolean, ProductosNE)), utilidad As String, tipo As String)
		Try
			...
			Dim type = ""
			If tipo = "porcentaje" Then 'Si es por porcentaje lo multiplico segun lo recibido
				type = "utilidad *" + utilidad
			ElseIf tipo = "monto" Then 'Si es utilidad exacto, lo igualo a lo solicitado
				type = utilidad.Replace(",", ".")
			Else 'Si es adicion lo agrego
				type = "utilidad +" + utilidad.Replace(",", ".")
			End If

			Dim control As New SqlCommand("update Productos set utilidad = " + type + " where id in(" + ids + ")", db)
			control.CommandType = CommandType.Text
			control.ExecuteNonQuery()
		Catch ex As Exception
			MsgBox(ex.Message, MsgBoxStyle.Critical, "Error")
	...
	Public Function CargaGrillaproductossinbusqueda(ByVal codigo As String, ByVal nombre As String, orderby As List(Of Tuple(Of Integer, String, Integer)), ascOrDesc As String, pagina As Integer, Optional esReparacion As String = "") As DataTable
		helpersDa.ChequearConexion(db)
		...
		sqlstr = If(esReparacion = "N", sqlstr + " and esParaReparacion = 'N'", sqlstr + " and esParaReparacion = 'S'")
